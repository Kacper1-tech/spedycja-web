...
export default function ListaFakturKosztowych() {
  ...
  const toggleSelect = (id) => {
    setFaktury(
      faktury.map((f) => (f.id === id ? { ...f, selected: !f.selected } : f)),
    );
  };
  ...
  const handleSave = async (id) => {
    ...
    setFaktury(
      faktury.map((f) =>
        f.id === id ? { ...f, ...editValues, isEditing: false } : f,
      ),
    );
  ...
  const handleEdit = (id) => {
    ...
    setFaktury(
      faktury.map((f) => (f.id === id ? { ...f, isEditing: true } : f)),
    );
  };
        ...
        <table className="min-w-full border table-fixed">
          <thead>
            ...
            <tr>
              ...
              {['nazwa', 'nrFaktury', 'PLN', 'EUR', 'GBP', 'dataPlatnosci'].map(
                (key) => (
