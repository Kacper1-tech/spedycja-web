...
export default function WszystkieZlecenia() {
  ...
  useEffect(() => {
    ...
    // ⏬ Od razu pobierz dane
    fetchData();

    // ⏬ Obsługa automatycznego odświeżania po aktualizacji z innych zakładek
    const refreshHandler = () => {
      fetchData();
    };

    window.addEventListener('refreshZleceniaExport', refreshHandler);
    window.addEventListener('refreshZleceniaImport', refreshHandler);
    window.addEventListener('refreshZleceniaPozostale', refreshHandler);

    return () => {
      window.removeEventListener('refreshZleceniaExport', refreshHandler);
      window.removeEventListener('refreshZleceniaImport', refreshHandler);
      window.removeEventListener('refreshZleceniaPozostale', refreshHandler);
    };
  }, []);
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (selectedZlecenie.typ === 'Export') {
                    ...
                    navigate(
                      `/zlecenia/pozostale/edytuj/${selectedZlecenie.id}`,
                    );
                  }
                }}
              >
